...
class pssfb_tests(unittest.TestCase):
    ...
    def test_inject_from_file_and_ask(self):
        ...
        for (key, val) in my_dictionary.iteritems():
            second[str(key)] = val
        expected = {'(polski//0, wisl//0)': set(['PPI']), \
            '(starostwo_graj//1, graj//0)': set(['PPI']), \
            '(wisl//0, wojewodztwo_mazowiecki//0)': set(['PO']), \
            '(wojewodztwo_wielkopolski//0, polski//0)': set(['PP']), \
            '(gali//0, niec//0)': set(['EQ']), \
            '(polski_bar//0, wojewodztwo_podlaski//0)': set(['PPI']), \
            '(szcz//0, graj//0)': set(['DR']), '(polski_bar//0, polski//0)': set(['PP']), \
            '(trzeci_rzesza//0, gali//0)': set(['PPI']), \
            '(trojmiasto//0, gdan//0)': set(['PPI']), '(wisl//0, trojmiasto//0)': set(['PO']), \
            ...
            '(wojewodztwo_podlaski//0, polski_bar//0)': set(['PP']), \
            '(wisl//0, wojewodztwo_malopolski//0)': set(['PO']), \
            '(trojmiasto//0, wisl//0)': set(['PO']), \
            '(starostwo_graj//0, szcz//0)': set(['PPI']), \
            '(polski//0, wojewodztwo_wielkopolski//0)': set(['PPI']), \
            '(wojewodztwo_podlaski//0, starostwo_graj//0)': set(['PPI']), \
            '(szcz//0, starostwo_graj//0)': set(['PP']), \
            '(trojmiasto//0, sopo//0)': set(['PPI']), '(gdan//0, trojmiasto//0)': set(['PP']), \
            '(wojewodztwo_mazowiecki//0, wisl//0)': set(['PO']), \
            '(graj//0, szcz//0)': set(['DR']), '(polski//0, polski_bar//0)': set(['PPI']), \
            '(sopo//0, trojmiasto//0)': set(['PP']), \
            '(wojewodztwo_malopolski//0, wisl//0)': set(['PO'])}
        ...
        answer = base._check_fact_("szcz", "PP", "polski")
        ...
        self.assertEqual("True", answer)
        answer = base._check_fact_("trojmiasto", "DR", "polski")
        # print "trojmiasto DR polski -> " + answer
        self.assertEqual("False", answer)
        answer = base._check_fact_("gdan", "PP", "polski")
